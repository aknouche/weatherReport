...
# Ask user for password to log in to sender's email
password = input("Please enter your email password: ")


# Sends an email (if the weather might be rainy). The function is called from print_weather()
def sendEmailRain():
    msg = MIMEMultipart()
...
# Sends an sms (if the weather might be rainy). The function is called from print_weather()
def sendSmsRain():
    try:
        ...
        message = twilioCli.messages.create(body='Risk för regn. Kom ihåg paraply och regnkläder!',
                                            from_=myTwilioNumber, to=myCellPhone)
    except:
        print("SMS was not sent")


# Sends an email (if the weather might be cold). The function is called from print_weather()
def sendEmailCold():
    msg = MIMEMultipart()
    msg['From'] = 'lamine.aknouche@malmo.se'
    msg['To'] = 'ml.aknouche@gmail.com'
...
# Sends an sms (if the weather might be cold). The function is called from print_weather()
def sendSmsCold():
    try:
        accountSID = 'AC620e24ce9259f3712bd893cf0391c8bf'
        authToken = '3fc430940fbc26b0bd9ec4e1428cc397'
        ...
        myTwilioNumber = '+46790645826'
        myCellPhone = '+46722515715'
        message = twilioCli.messages.create(body='Det kan bli kallt, ta på varma kläder! Skippa cykeln',
                                            from_=myTwilioNumber, to=myCellPhone)
    except:
        print("SMS was not sent")


# Creates the URL depending on the query returned from main (that is q+city)
